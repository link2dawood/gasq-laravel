<?php

namespace Tests\Unit;

use App\Services\V24\MobilePatrol\MobilePatrolV24ComputeService;
use PHPUnit\Framework\Attributes\DataProvider;
use Tests\TestCase;

class MobilePatrolV24ParityTest extends TestCase
{
    #[DataProvider('cases')]
    public function test_mobile_patrol_v24_cases_match_expected(string $caseFile): void
    {
        $in = json_decode(file_get_contents($caseFile), true, flags: JSON_THROW_ON_ERROR);
        $expectedFile = str_replace('.json', '.expected.json', $caseFile);
        $expected = json_decode(file_get_contents($expectedFile), true, flags: JSON_THROW_ON_ERROR);

        /** @var MobilePatrolV24ComputeService $svc */
        $svc = $this->app->make(MobilePatrolV24ComputeService::class);
        $out = $svc->compute($in['scenario']);

        $this->assertSame($expected['kpis'], $out['kpis']);
    }

    public static function cases(): array
    {
        $base = base_path('tests/Fixtures/v24/mobile_patrol');
        return [
            'basic' => [$base.'/case_basic.json'],
        ];
    }
}

