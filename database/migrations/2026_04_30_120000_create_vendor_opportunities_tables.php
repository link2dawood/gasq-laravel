...
return new class extends Migration
{
    public function up(): void
    {
        if (! Schema::hasTable('vendor_opportunities')) {
            Schema::create('vendor_opportunities', function (Blueprint $table) {
                ...
                $table->unique('job_posting_id', 'vo_job_unique');
                $table->index(['lead_tier', 'status'], 'vo_tier_status_idx');
            });
        }
        ...
        if (! Schema::hasTable('vendor_opportunity_invitations')) {
            Schema::create('vendor_opportunity_invitations', function (Blueprint $table) {
                ...
                $table->string('invite_key', 64)->unique('voi_invite_unique');
                ...
                $table->unique(['vendor_opportunity_id', 'vendor_id'], 'voi_vendor_unique');
                $table->index(['status', 'sent_at'], 'voi_status_sent_idx');
            });
        }
    }
    ...
    {
        if (Schema::hasTable('vendor_opportunity_invitations')) {
            Schema::drop('vendor_opportunity_invitations');
        }

        if (Schema::hasTable('vendor_opportunities')) {
            Schema::drop('vendor_opportunities');
        }
    }
};
